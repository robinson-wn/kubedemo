...
dataset_partitions = 4

data_url = "http://data.insideairbnb.com/united-states/ca/san-francisco/2021-12-04/data/listings.csv.gz"
...
get_spark().sparkContext.setLogLevel("ERROR")
logging.getLogger('py4j.clientserver').setLevel(logging.WARNING)
get_spark().conf.set("spark.sql.shuffle.partitions", "8")
...
def single_pipeline_multiple_regressions(params: Dict = None, folds=2, metric='r2', limit=None):
    ...
    if params:
        ...
        regression_params = [{'elasticNetParam': [0.5, 1.0]}, {'regParam': [0.5, 1.0]}, {'minInfoGain': [0.0, 0.3]}]
    ...
    cv = CrossValidator() \
        ...
        .setNumFolds(folds)
    logger.info("Fitting all parameteirzed models to data.")
    if limit:
...
def create_dataset(url=data_url, file=data_file):
    ...
    df = get_spark().createDataFrame(pdf)
